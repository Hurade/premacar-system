-- ============================================================
-- Permite que imports em massa (ex: backup histórico de outro CRM)
-- pulem a criação automática de deal/round-robin ao inserir contatos
-- ou conversas — sem essa flag, toda importação em massa gera 1 deal
-- (com dono sorteado por round-robin) por linha inserida.
--
-- Uso em qualquer script de import futuro, ANTES do INSERT em massa:
--   SET LOCAL app.skip_deal_trigger = 'true';
--   INSERT INTO public.contacts (...) VALUES (...);
-- (SET LOCAL vale só pra transação atual — não precisa "religar" depois)
-- ============================================================
