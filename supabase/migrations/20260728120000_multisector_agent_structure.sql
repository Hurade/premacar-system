...
--
-- Até aqui o SDR Prema atendia só o funil comercial da Prema Car com um
-- único agente. Esta migration prepara o terreno para atender TODO o
-- atendimento (Comercial, Suporte, CS, Financeiro, RH) dos 4 sistemas da
-- empresa (Prema Car, Automax Frotas, Automax Oficina, Maxsig) pela mesma
-- plataforma, com múltiplas conexões WhatsApp e múltiplos atendentes por
-- fila.
--
-- Reaproveita o mecanismo de seleção de agente que já existe
-- (agent_configs.trigger_type: default > queue > campaign) — não recria
-- nada, só estende: contexto de sistema/marca por conexão, RAG segmentado
-- por fila, atendentes/telefone de notificação reais por fila (substitui o
-- enum team_assignment, que tinha nomes de pessoas hardcoded em vez de
-- setores), e provedor de IA configurável (hoje só existe Gemini via
-- gateway Lovable, hardcoded).
-- ============================================================
...
CREATE TABLE public.systems (
  ...
  name TEXT NOT NULL,
  greeting_label TEXT NOT NULL, -- como a Cris se apresenta: "aqui é a Cris da {{ greeting_label }}"
...
-- ------------------------------------------------------------
-- 2. Conexão ↔ Sistema (N:N — uma conexão pode representar mais de um
--    sistema, ex: "Geral Automax" atende Frotas + Oficina + Maxsig)
-- ------------------------------------------------------------
...
-- Mapeamento das conexões existentes fica a cargo do usuário pela tela de
-- Conexões (novo seletor de sistemas) — não é seguro adivinhar aqui qual
-- linha de whatsapp_connections corresponde a qual número real.

-- ------------------------------------------------------------
-- 3. Filas novas (Comercial/Suporte/Financeiro já existem desde
--    20260709120000_create_queues_and_favorites.sql) — adiciona CS e RH.
--    "Atendimento" não precisa de fila própria: é o agente trigger_type
--    ='default', roda antes de qualquer fila ser atribuída.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 4. Atendentes por fila — substitui o enum team_assignment (nomes de
--    pessoas hardcoded) por um vínculo real fila ↔ pessoa, com telefone de
--    notificação. Uma fila pode ter mais de um atendente.
-- ------------------------------------------------------------
...
-- Nota: conversations.assigned_team (enum team_assignment) fica sem uso
-- novo a partir desta migration — não é removido agora para não descartar
-- histórico sem necessidade, mas nenhuma edge function volta a escrever
-- nele. queue_id + queue_members é a fonte de verdade daqui para frente.

-- ------------------------------------------------------------
-- 5. Classificação de origem do lead (Comercial) — persistida pela tool
--    transfer_to_human em vez do heurístico calculado em runtime
--    (origem_conversa), que nunca era gravado de fato.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 6. RAG segmentado por fila — Suporte e CS não vazam conteúdo um no
--    outro. NULL continua significando "global/compartilhado".
-- ------------------------------------------------------------
...
CREATE OR REPLACE FUNCTION public.match_documents(
  ...
  WHERE kd.status = 'ready'
    ...
    AND (
      (p_queue_id IS NULL AND kd.queue_id IS NULL) -- sem fila (ex: agente Atendimento) só vê base global
...
-- ------------------------------------------------------------
-- 7. Provedor de IA configurável — hoje só existe o gateway Lovable
--    (Gemini) hardcoded em nina-orchestrator. Passa a ser uma tabela, com
--    o gateway atual seedado como provider default para não quebrar nada
--    em produção.
-- ------------------------------------------------------------
CREATE TABLE public.ai_providers (
  ...
  api_key_secret_name TEXT NOT NULL, -- nome da env var/secret da edge function, nunca a chave em texto puro
...
ALTER TABLE public.agent_configs
  ...
  ADD COLUMN ai_model TEXT; -- override explícito do modelo; se vazio, usa model_mode (flash/pro/pro3/adaptive) contra o provider selecionado

-- ------------------------------------------------------------
-- 8. Agente Atendimento — reaproveita a linha trigger_type='default' já
--    garantida única pela migration 20260710120000_unify_agent_configs.sql,
--    só troca nome/descrição/prompt (deixa de ser o pitch de vendas da
--    Prema Car e passa a ser o roteador neutro por setor).
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 9. Os 5 agentes de fila (Comercial/Suporte/CS/Financeiro/RH)
-- ------------------------------------------------------------
...
-- Backfill: agentes já existentes (ex: o "Atendimento" atualizado acima)
-- ficam sem provider explícito até aqui — aponta todos para o default,
-- para não deixar nenhum agente sem provider configurado.
UPDATE public.agent_configs
