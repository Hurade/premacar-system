...
const componentIcons: Record<string, React.ReactNode> = {
  ai_gateway: <Shield className="w-5 h-5" />,
...
const componentLabels: Record<string, string> = {
  ai_gateway: 'Chave de IA',
  whatsapp: 'WhatsApp (Evolution ou Meta)',
...
export const StepVerification: React.FC<StepVerificationProps> = ({ onAllChecked }) => {
    ...
    try {
      ...
      if (data) {
        ...
        const criticalOk = data.results
          .filter(r => ['ai_gateway', 'nina_settings', 'pipeline_stages'].includes(r.component))
  ...
  const criticalResults = results.filter(r => 
    ['ai_gateway', 'nina_settings', 'pipeline_stages'].includes(r.component)
  );
